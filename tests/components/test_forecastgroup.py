...
class TestForecastGroup(unittest.TestCase):
    ...
    def _test_tempo_group(self, raw, start_time, end_time, **kwargs):
        group = aviation_weather.TempoGroup(raw)
        self.assertEqual(start_time, group.start_time)
        self.assertEqual(end_time, group.end_time)
        self._test_forecast_group(raw, group, **kwargs)

    def test_tempo_group_1(self):
        self._test_tempo_group(
    ...
    def _test_prob_group(self, raw, probability, start_time, end_time, **kwargs):
        group = aviation_weather.ProbGroup(raw)
        self.assertEqual(probability, group.probability)
        self.assertEqual(start_time, group.start_time)
        self.assertEqual(end_time, group.end_time)
        self._test_forecast_group(raw, group, **kwargs)

    def test_prop_group_1(self):
        self._test_prob_group(
    ...
    def _test_becmg_group(self, raw, start_time, end_time, **kwargs):
        group = aviation_weather.BecmgGroup(raw)
        self.assertEqual(start_time, group.start_time)
        self.assertEqual(end_time, group.end_time)
        self._test_forecast_group(raw, group, **kwargs)

    def test_becmg_group_1(self):
        self._test_becmg_group(
